...
STOPS_FILE="/Users/mac/Desktop/ParisCité_M1/Adm_Linux/project/updates/metro_ids.txt"

# Define the directory to store the output file
OUTPUT_DIR="/Users/mac/Desktop/ParisCité_M1/Adm_Linux/project/retrieved_data/metro_next_departures"
...
while IFS= read -r stop_id; do
    ...
    next_departures=$(curl -sf "${API_URL}?MonitoringRef=STIF:StopPoint:Q:${stop_id}:" -H "apikey: ${API_KEY}" 2>&1)

    if [ $? -eq 0 ]; then
        NEXT_DEPARTURES+=("$next_departures")
    else
        echo "Error fetching data for stop $stop_id" >&2
    fi
done < "$STOPS_FILE"
